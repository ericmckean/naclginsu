...
extern void DebugEvent( struct GLUtesselator *tess );
#else
...
static void SweepEvent( struct GLUtesselator *tess, GLUvertex *vEvent );
static void WalkDirtyRegions( struct GLUtesselator *tess, ActiveRegion *regUp );
static int CheckForRightSplice( struct GLUtesselator *tess, ActiveRegion *regUp );

static int EdgeLeq( struct GLUtesselator *tess, ActiveRegion *reg1,
		    ActiveRegion *reg2 )
/*
...
static void DeleteRegion( struct GLUtesselator *tess, ActiveRegion *reg )
{
  if( reg->fixUpperEdge ) {
...
static ActiveRegion *AddRegionBelow( struct GLUtesselator *tess,
...
static GLboolean IsWindingInside( struct GLUtesselator *tess, int n )
{
...
static GLUhalfEdge *FinishLeftRegions( struct GLUtesselator *tess,
...
static void AddRightEdges( struct GLUtesselator *tess, ActiveRegion *regUp,
...
static void CallCombine( struct GLUtesselator *tess, GLUvertex *isect,
...
static void SpliceMergeVertices( struct GLUtesselator *tess, GLUhalfEdge *e1,
				 GLUhalfEdge *e2 )
/*
...
static void GetIntersectData( struct GLUtesselator *tess, GLUvertex *isect,
...
static int CheckForLeftSplice( struct GLUtesselator *tess, ActiveRegion *regUp )
/*
 * Check the upper and lower edge of "regUp", to make sure that the
...
static int CheckForIntersect( struct GLUtesselator *tess, ActiveRegion *regUp )
/*
...
static void ConnectRightVertex( struct GLUtesselator *tess, ActiveRegion *regUp,
...
static void ConnectLeftDegenerate( struct GLUtesselator *tess,
...
static void ConnectLeftVertex( struct GLUtesselator *tess, GLUvertex *vEvent )
/*
...
static void AddSentinel( struct GLUtesselator *tess, GLdouble t )
/*
...
static void InitEdgeDict( struct GLUtesselator *tess )
/*
...
static void DoneEdgeDict( struct GLUtesselator *tess )
{
  ActiveRegion *reg;
  int fixedEdges = 0;
...
static void RemoveDegenerateEdges( struct GLUtesselator *tess )
/*
...
static int InitPriorityQ( struct GLUtesselator *tess )
/*
...
static void DonePriorityQ( struct GLUtesselator *tess )
{
  pqDeletePriorityQ( tess->pq ); /* __gl_pqSortDeletePriorityQ */
}
...
int __gl_computeInterior( struct GLUtesselator *tess )
/*
