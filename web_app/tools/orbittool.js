...
/**
 * @fileoverview  Implement a virtual trackball in the tools.OrbitTool
...
goog.provide('ginsu.tools.OrbitTool');
...
/**
 * Constructor for the OrbitTool object.  This class maps 2D mouse drag events
...
ginsu.tools.OrbitTool = function() {
   ...
   * @private
   */
  this.rollStart_ = new goog.math.Vec3(0, 0, 1);
...
goog.inherits(ginsu.tools.OrbitTool, ginsu.tools.Tool);
 ...
 * @private
 */
ginsu.tools.OrbitTool.prototype.initInFrame_ = function(frameSize) {
 ...
 * @param {!goog.math.Coordinate} clientPoint map this point to the corrdinate
 *     space with origin in thecenter of the client view.
 * @return {goog.math.Coordinate} the converted point.
 * @private
 */
ginsu.tools.OrbitTool.prototype.convertClientPoint_ =
    function(clientPoint) {
  return new goog.math.Coordinate.difference(clientPoint, this.center_);
};
 ...
 * is set to 0.
 * @param {!goog.math.Coordinate} point 2D-point in the coordinate space with
 *     origin in the center of the client view.
 * @return {goog.math.Vec3} the 3D point on the virtual trackball.
 * @private
 */
ginsu.tools.OrbitTool.prototype.projectOnTrackball_ = function(point) {
  ...
  var trackballPoint = new goog.math.Vec3(point.x, point.y, zValue);
 ...
 *     element that encloses the virtual trackball.
 */
ginsu.tools.OrbitTool.prototype.startAtPointInFrame =
 ...
 * |rollEnd_|.
 * @param {!goog.math.Coordinate} dragPoint 2D-point representing the
 ...
 *     sin(angle/2).
 */
ginsu.tools.OrbitTool.prototype.rollToPoint = function(dragPoint) {
  var rollTo = this.convertClientPoint_(dragPoint);
  if (goog.math.nearlyEquals(this.rollStart_.x, rollTo.x,
                             ginsu.tools.OrbitTool.DOUBLE_EPSILON) &&
      goog.math.nearlyEquals(this.rollStart_.y, rollTo.y,
                             ginsu.tools.OrbitTool.DOUBLE_EPSILON)) {
  ...
  var rollVector = goog.math.Vec3.cross(this.rollStart_, rollEnd);
  var invStartMag = 1.0 / this.rollStart_.magnitude();
  var invEndMag = 1.0 / rollEnd.magnitude();

  // cos(a) = (s . e) / (||s|| ||e||)
  var cosAng =
      goog.math.Vec3.dot(this.rollStart_, rollEnd) * invStartMag * invEndMag;
  // sin(a) = ||(s X e)|| / (||s|| ||e||)
  ...
  // imaginary part of the quaternion and is computed as [x, y, z] *
  // sin(angle/2).
  var imaginaryPart = rollVector.scale(Math.sin(rollHalfAngle));
  var ballQuaternion = [imaginaryPart.x, imaginaryPart.y,
                        imaginaryPart.z, Math.cos(rollHalfAngle)];
 ...
 * @param {!ginsu.controllers.ViewController} view The view controller that
 *     called this method.
 * @param {!goog.fx.DragEvent} dragStartEvent The START event that
 *     triggered this handler.
 */
ginsu.tools.OrbitTool.prototype.handleStartDrag =
  ...
  var flippedY = frame.height - dragStartEvent.clientY;
  this.startAtPointInFrame(
      new goog.math.Coordinate(dragStartEvent.clientX, flippedY),
      new goog.math.Size(frame.width, frame.height));
};
...
ginsu.tools.OrbitTool.prototype.handleDrag =
  ...
  view.setValueForKey(
      ginsu.tools.OrbitTool.CAMERA_ORIENTATION_KEY,
      ginsu.multQuaternions(this.cameraOrientation_,
          this.rollToPoint(new goog.math.Coordinate(
              dragEvent.clientX, flippedY))));
};
 ...
 */
ginsu.multQuaternions = function(q0, q1) {
 ...
 * @type {number}
 */
ginsu.tools.OrbitTool.DOUBLE_EPSILON = 1.0e-16;
